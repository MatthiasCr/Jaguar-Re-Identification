...
class ArcFaceModel(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if freeze_last_n_layers is not None:
            if train_last_n_layers not in (0, freeze_last_n_layers):
                raise ValueError("Use either train_last_n_layers or freeze_last_n_layers, not both")
            train_last_n_layers = freeze_last_n_layers
